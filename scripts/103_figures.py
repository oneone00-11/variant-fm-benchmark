...
ax.set_yticklabels([f"{m}  (n={covn.get(m,0)})" for m in sp.model])
...
plt.tight_layout()
plt.savefig(RESULTS_FIGURES / "fig1_splice_performance.png", dpi=150)
plt.close()
...
ax.legend()
plt.tight_layout()
plt.savefig(RESULTS_FIGURES / "fig2_forest_spliceai_splice.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(RESULTS_FIGURES / "fig3_region_heatmap.png", dpi=150)
plt.close()
